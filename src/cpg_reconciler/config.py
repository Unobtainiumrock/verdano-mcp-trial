...
1. ``$CPG_RECONCILER_PROJECT_ROOT/.env``
...
def _find_env_file() -> str:
    """Locate the .env file, preferring CPG_RECONCILER_PROJECT_ROOT."""
    root_env = os.environ.get("CPG_RECONCILER_PROJECT_ROOT")
    if root_env:
...
class Settings(BaseSettings):
    ...
    duckdb_path: str = Field(default=".local/cpg_reconciler.duckdb")
    ...
    def _compat_aliases(cls, values: dict) -> dict:  # type: ignore[type-arg]
        """Accept legacy CPG_RECONCILER_E4_MIN_SCORE as an alias."""
