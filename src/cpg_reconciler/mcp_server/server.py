...
Run via `python -m cpg_reconciler.mcp_server` (stdio transport) for use under an MCP
...
from cpg_reconciler.canonical import RetailerCode, validate_retailer_code
from cpg_reconciler.config import Settings, load_settings
from cpg_reconciler.erp import Client
from cpg_reconciler.llm.client import LLMClient, create_llm_client
from cpg_reconciler.mapping.depot import resolve_depot
from cpg_reconciler.mapping.hooks import ConfidenceHook, temperature_band_penalty
from cpg_reconciler.mapping.normalize import NormalizationPipeline, build_pipeline
...
from cpg_reconciler.pipeline.drafts import create_drafts
from cpg_reconciler.storage import AuditEntry, DuckDBRepository, InMemoryRepository, Repository
...
def _resolve_project_root() -> Path:
    ...
    env_root = os.environ.get("CPG_RECONCILER_PROJECT_ROOT")
...
def build_server(
    ...
    server: FastMCP = FastMCP("cpg-reconciler")
...
def _run_health_check(project_root: Path) -> int:
    ...
    try:
        ...
        has_key = False
    checks.append(("API key configured", has_key, "CPG_RECONCILER_ERP_API_KEY"))
...
def run() -> None:
    """Entry-point for `cpg-reconciler` / `python -m cpg_reconciler.mcp_server`."""
